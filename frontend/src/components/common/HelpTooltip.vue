...
<template>
  ...
  >
    ...
    <slot name="trigger">
      <svg
        class="app-theme-readable-muted h-4 w-4 cursor-help transition-colors hover:text-primary-600 dark:hover:text-primary-400"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
    ...
    <Teleport to="body">
      ...
      >
        <button
          ...
          type="button"
          class="app-theme-tooltip-close absolute right-1.5 top-1.5 rounded p-1 transition-colors"
          aria-label="Close"
        ...
        <slot>{{ content }}</slot>
        <div class="app-theme-tooltip-arrow absolute -bottom-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45"></div>
      </div>
    </Teleport>
  </div>
</template>
