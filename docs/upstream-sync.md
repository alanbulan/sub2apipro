...
The scheduled wrapper is designed for the 4GB production server. When upstream commits are pending, it runs Codex in a systemd transient service with bounded memory, swap, process count, and runtime. The production server is never a build or test host: scheduled and manual production runs may use only low-cost static checks such as `git diff --check`, `bash -n`, and `sh -n`. Docker builds, frontend production builds, backend tests, Go tests, TypeScript compilation, package-manager scripts, and development servers are performed only by GitHub Actions.

Applied updates are first pushed to `sync/upstream-candidate`. The wrapper waits up to 45 minutes for that branch's GitHub `CI` workflow. It promotes the exact candidate commit to `origin/main` only after CI succeeds; the existing `Build and Deploy` workflow then builds, publishes, and deploys it. A failed, cancelled, or timed-out candidate CI leaves `main` and `last-seen-head` unchanged, while retaining the candidate branch and its Action log for diagnosis. The wrapper uses the public GitHub API by default and may use `GITHUB_TOKEN` when the repository requires authentication.
...
An interrupted, failed, blocked, or CI-rejected review does not advance `.codex-upstream-sync/last-seen-head`. The next scheduled run will review the same upstream commits. A successful `noop` review advances the state only after the complete review proves that no code change is required.
