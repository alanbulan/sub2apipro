...
包装脚本会在调用合并代理前写入 `in-progress-head`。若进程在提交候选前异常
中断，下次执行会先核对该基线、当前分支和提交祖先关系，再恢复自动化拥有的
未完成合并。没有进程持有且超过 5 分钟的 `.git/index.lock` 不会直接删除，
而会移动到 `.codex-upstream-sync/stale-index-lock.*` 后再恢复；新产生或仍被
进程持有的锁会停止任务并保留现场。候选检查点已经存在时，仍优先续接同一
候选，不重新合并。
